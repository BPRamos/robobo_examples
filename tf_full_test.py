...
def write_detection(file, detection, labels):
    file.write(u"Bbox:\n")
    ...
    for result in detection.results:
        class_id = labels[int(result.id)+1]
        file.write(u"\t{}:\n".format(j))
...
def create_experiment(path, objects, distances, pans, tilts):
    ...
    exit_code = subprocess.call(["python3", "tf_full_test_gen_exp.py", "-path", str(path)])
...
def write_report(experiment_path, notes, objects, distances, tilts, pans, has_pan, has_tilt, experiments):
    ...
    with (experiment_path / "report.txt").open(mode="w+") as f:
        ...
        f.write(u"Experiment began at {}".format(now.strftime("%Y-%m-%d %H:%M:%S")))
...
def write_report_coda(experiment_path):
    with (experiment_path / "report.txt").open(mode="a") as f:
        now = datetime.datetime.now()
        f.write(u"Experiment ended at {}".format(now.strftime("%Y-%m-%d %H:%M:%S")))

class tf_experiment_node(object):
    def __init__(self):
        ...
        rospy.init_node('tf_experiment_node', anonymous=True)

        self.spawn_service = ("/gazebo/spawn_sdf_model", SpawnModel)
        self.delete_service = ("/gazebo/delete_model", DeleteModel)
        #self.pan_tilt_serv_client = get_service_client("/robot/robobo/movePanTilt", MovePanTilt)
        self.pan_tilt_service = ("/robot/robobo/movePanTilt", MovePanTilt)
        
        rospy.Subscriber("/robot/robobo/detected_object", Detection2DArray, self.detection_callback)
        rospy.Subscriber("/robot/robobo/camera/image", Image, self.camera_callback)
        #debug
        rospy.Subscriber("/robot/robobo/debug_detected_object_img", Image, self.debug_img_callback)

        self.run_experiments()
        rospy.signal_shutdown("tf_test finished recording. Run again for a new recording")

    def debug_img_callback(self, data):
        self.debug_img = data
    ...
    def write_image(self, path, threshold=0.35):
        ...
        debug_img = self.cv_bridge.imgmsg_to_cv2(self.debug_img, desired_encoding="rgb8")
        cv2.imwrite(str(path / "original.png"), cv_image)
        cv2.imwrite(str(path / "debug.png"), debug_img)
    ...
    def write_experiment(self,exp_path, obj_name, index):
        ...
        detection2DArray = self.detection2DArray
        i = 0
        for detection in detection2DArray.detections:
            file = path / "{}.detection".format(i)
            with file.open(mode="w+") as f:
                write_detection(f, detection, self.labels)
            i+=1
    ...
    def run_experiments(self):
        ...
        now = datetime.datetime.now()
        experiment_path = pathlib2.Path("./experiments/{}".format(now.strftime("%d-%m-%Y_%H%M%S")))
        experiment_path.mkdir(parents=True, exist_ok=True)

        """
        objects = [ ("apple", "./../../robobo-gazebo-models/apple/model.sdf"),
                    ("dog", "./../../robobo-gazebo-models/dog/model.sdf"),
                    ("glass", "./../../robobo-gazebo-models/glass/model.sdf"),
                    ("bottle", "./../../robobo-gazebo-models/bottle/model.sdf")
                    ]
        """
        """
        objects = [ ("velocidad50_qr", "./../../robobo-gazebo-models/velocidad50_qr/model.sdf"),
        ...
        objects = [ ("velocidad50_qr", "./../../robobo-gazebo-models/velocidad50_qr/model.sdf"),
                    ...
                    ("izquierda_qr", "./../../robobo-gazebo-models/izquierda_qr/model.sdf")
                    ]

        #distances = [0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4, 2.7, 3.0 ]
...
def main():
    instancia = tf_experiment_node()
    try:
        rospy.spin() # spin() simply keeps python from exiting until this node is stopped
    except rospy.ROSInterruptException:
        print("Shutting down crap test module")
